...
export const sections: DeckSection[] = [
  ...
  { label: 'What we commit to', short: 'Commitments', start: 33, end: 34 },
  { label: 'Results', short: 'Results', start: 35, end: 36 },
  { label: 'Next step', short: 'Next step', start: 37, end: 38 },
];

export const sectionForIndex = (i: number): DeckSection =>
  sections.find((s) => i >= s.start && i <= s.end) ?? sections[0];

export const TOTAL_SCREENS = 39;
export const ROADMAP_INDEX = 37;
